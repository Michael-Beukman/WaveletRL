...
class AWRCandidateGroups:
    """
    This class contains the information to keep track of a collection of functions that are currently in the basis, as well as their children (i.e. split) functions.
    
    This updates the relevances and also provides should_split function to indicate if a split should indeed take place.
    """
    verbose = False
    """
    This class needs to have the current functions, as well as the candidates, and all of their relevances
    """
    ...
    def add_function(self, index: int, new_funcs: List[List[AWRBasisFunction]]):
        """Performs the addition of functions to the basis set, and then updates their children, etc.

        Args:
            index (int): Where to add
            new_funcs (List[List[AWRBasisFunction]]): This should have shape (1, X), where X is the number of children
